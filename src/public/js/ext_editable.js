...
(function (factory, window) {
  ...
  L.Editable.VertexMarker = L.Marker.extend({
    ...
    onDrag: function (e) {
      ...
      var latlng = this._map.layerPointToLatLng(iconPos)
  ...
  L.Editable.MiddleMarker = L.Marker.extend({
    ...
    setVisibility: function () {
      ...
      var rightPoint = this._map.latLngToContainerPoint(this.right.latlng)
      var size = L.point(this.options.icon.options.iconSize)
    ...
    computeLatLng: function () {
      ...
      var rightPoint = this.editor.map.latLngToContainerPoint(this.right.latlng)
    ...
    onMouseDown: function (e) {
      var iconPos = L.DomUtil.getPosition(this._icon)
      var latlng = this.editor.map.layerPointToLatLng(iconPos)
      e = {
        originalEvent: e,
        latlng: latlng
      }
  ...
  L.Editable.PathEditor = L.Editable.BaseEditor.extend({
    ...
    _deleteShape: function (shape, latlngs) {
      ...
      var self = this
      var inplaceDelete = function (latlngs, shape) {
                    // Called when deleting a flat latlngs
        shape = latlngs.splice(0, Number.MAX_VALUE)
        return shape
      }
      var spliceDelete = function (latlngs, shape) {
  ...
  L.Editable.PolygonEditor = L.Editable.PathEditor.extend({
    ...
    vertexCanBeDeleted: function (vertex) {
      ...
      var idx = L.Util.indexOf(parent, vertex.latlngs)
  ...
  L.Editable.RectangleEditor = L.Editable.PathEditor.extend({
    ...
    extendBounds: function (e) {
      var index = e.vertex.getIndex()
      var next = e.vertex.getNext()
      var previous = e.vertex.getPrevious()
      var oppositeIndex = (index + 2) % 4
      var opposite = e.vertex.latlngs[oppositeIndex]
    ...
    updateLatLngs: function (bounds) {
      var latlngs = this.getDefaultLatLngs()
      var newLatlngs = this.feature._boundsToLatLngs(bounds)
            // Keep references.
      for (var i = 0; i < latlngs.length; i++) {
  ...
  L.Editable.CircleEditor = L.Editable.PathEditor.extend({
    ...
    computeResizeLatLng: function () {
      ...
      var point = this.map.project(this.feature._latlng)
    ...
    // `EditableMixin` is included to `L.Polyline`, `L.Polygon`, `L.Rectangle`,
    // `L.Circle` and `L.Marker`. It adds some methods to them.
